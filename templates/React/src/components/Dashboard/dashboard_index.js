...
/**
 * Component for rendering the dashboard index.
 * @param {Object} props - Component props.
 * @param {React.ReactNode} props.children - The child components to render.
 * @returns {JSX.Element} - Dashboard index component.
 */
export default function DashboardIndex(props) {
    ...
    return (
        <>
            {["post_detail", "company_profile", "post_Create", "companyVisitStats", "company_create", "company_visiting_create"].every(
                (ev) => window.location.href.search(ev) === -1
            ) === true ? (
                <div className="justify-center items-center mt-2 ml-3 ">
                    <div className="grid grid-rows-4 grid-cols-4 gap-6 gap-y-4 h-screen">
                        <div className="hidden lg:block row-span-3 h-screen">
                            <div className="border-2 border-grey-100 flex flex-col justify-between py-4 px-3 rounded">
                                                                ...
                                                                <div
                                                                    ...
                                                                    className={` m-4  flex items-center ${
                                                                        window.location.pathname.includes(ev.url_include)
                                                                            ...
                                                                            : "hover:text-white"
                                                                    }  hover:bg-indigo-600  rounded-xl  text-sm text-black-300 py-2 px-5`}
                                                                >
            ...
            <div className="h-7 row-span-3"></div>
        </>
    );
}
